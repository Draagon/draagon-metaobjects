...
public class SimpleMappingHandlerDB implements MappingHandler {
	 ...
	 * Enhanced to check PrimaryKey metadata for auto-increment configuration.
	 */
	protected String getAutoGenerated( MetaField mf ) {
		...
		// NEW: Check if this field is part of a PrimaryKey with auto-increment configuration
		MetaObject parentObject = (MetaObject) mf.getParent();
		if (parentObject != null) {
			// Look for PrimaryKey metadata children
			for (PrimaryKey primaryKey : parentObject.getChildren(PrimaryKey.class)) {
				// Check if this MetaField is part of this primary key
				if (primaryKey.getKeyFields().contains(mf)) {
					// Check for auto-increment strategy
					String strategy = primaryKey.getAutoIncrementStrategy();
					if (strategy != null) {
						// Map PrimaryKey auto-increment strategies to database layer values
						switch (strategy) {
							case PrimaryKey.AUTO_INCREMENT_SEQUENTIAL:
								return AUTO_LAST_ID; // "last" -> database auto-increment (Derby IDENTITY, etc.)
							case PrimaryKey.AUTO_INCREMENT_UUID:
								return AUTO_LAST_ID; // "last" -> UUID generation
							case PrimaryKey.AUTO_INCREMENT_NONE:
							default:
