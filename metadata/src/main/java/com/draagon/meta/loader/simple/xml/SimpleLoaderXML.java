...
import com.draagon.meta.loader.simple.SimpleModelParser;
import com.draagon.meta.loader.simple.SimpleTypesParser;
...
public class SimpleLoaderXML extends MetaDataLoader {
    ...
    public SimpleLoaderXML(String name) {
        super(LoaderOptions.create( false, false, true), SUBTYPE_SIMPLE, name );
    }

    public static SimpleLoaderXML createManual( String name, String resource ) {
        return createManualURIs( name, Arrays.asList(URIHelper.toURI( "model:resource:"+resource)));
    }

    public static SimpleLoaderXML createManual( String name, List<String> resources ) {
        List<URI> uris = new ArrayList<>();
        for (String s : resources) uris.add(URIHelper.toURI("model:resource:"+s));
        return createManualURIs( name, uris);
    }

    public static SimpleLoaderXML createManualURIs( String name, List<URI> uris ) {

        SimpleLoaderXML simpleLoader = new SimpleLoaderXML( name );
    ...
    @Override
    public SimpleLoaderXML setMetaDataClassLoader( ClassLoader classLoader ) {
        return super.setMetaDataClassLoader( classLoader );
    }

    public SimpleLoaderXML setSourceURIs( List<URI> sourceData ) {
    ...
    public SimpleLoaderXML init() {
        ...
        for( URI sourceURI : sourceURIs) {
            ...
            if (URIHelper.isTypesURI(sourceURI)) {
                SimpleTypesParserXML simpleTypesParser = new SimpleTypesParserXML(
                        getTypesLoader(),
                        getMetaDataClassLoader(),
                        sourceURI.toString() );
                ...
                SimpleModelParserXML simpleModelParser = new SimpleModelParserXML(
