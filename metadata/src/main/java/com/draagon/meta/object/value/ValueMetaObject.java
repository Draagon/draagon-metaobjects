...
public class ValueMetaObject extends PojoMetaObject
{
    ...
    protected Class<?> getObjectClass() throws ClassNotFoundException {
        try { 
            ...
            if (hasMetaAttr(MetaObject.ATTR_OBJECT)|| hasMetaAttr(MetaObject.ATTR_CLASS)) {
                throw e;
            }
    ...
    private ValueObject getMetaObject(Object o) {
        ...
        if (o == null) {
            throw new MetaDataException("ValueObject expected, but was bnull");
        }

        if (!(o instanceof ValueObject)) {
            throw new MetaDataException("ValueObject expected, not [" + o.getClass().getName() + "]");
        }
