...
public class ConfigLoaderTest {
    ...
    public void testMetaDataLoader() throws IOException, MetaDataIOException {
        ...
        MetaDataModelLoader loader = MetaDataModelLoader.create( "test-model-roundtrip");
        ...
        MetaDataAware<MetaObject> metadata = (MetaDataAware<MetaObject>) XMLObjectReader.readObject( MetaDataModel.class,
                loader.getMetaObjectByName(MetaDataModel.OBJECT_NAME), inputStream );
    ...
    public void testSimpleLoader() throws IOException, MetaDataIOException {
        ...
        MetaDataModelLoader loader = MetaDataModelLoader.create( "test-simple-roundtrip");

        final String TEST_METADATA_XML = "com/draagon/meta/loader/simple/fruitbasket-metadata.xml";

        SimpleLoader simpleLoader = SimpleLoader.createManual( loader.getName(), TEST_METADATA_XML );

        MetaDataModel metadata = buildModelFromLoader(
                loader.getMetaObjectByName( MetaDataModel.OBJECT_NAME ), simpleLoader );
    ...
    public static MetaDataModel buildModelFromLoader( MetaObject modelMetaObject, SimpleLoader simpleLoader) {

        MetaDataModel m = (MetaDataModel) modelMetaObject.newInstance();
        populateModel( m, simpleLoader, true );

        return m;
    }

    private static void populateModel(MetaDataModel m, MetaData<?> md, boolean isRoot) {

        if (isRoot) {
            m.setString( MetaDataModel.FIELD_TYPE, MetaDataModel.OBJECT_NAME);
        }
        else {
            m.setString( MetaDataModel.FIELD_PACKAGE, _trimToNull( md.getPackage() ));
            m.setString( MetaDataModel.FIELD_TYPE, md.getTypeName() );
            m.setString( MetaDataModel.FIELD_SUBTYPE, md.getSubTypeName() );
            m.setString( MetaDataModel.FIELD_NAME, md.getShortName() );
            ...
            if ( md.getSuperData() != null ) {
                ...
                m.setString(MetaDataModel.FIELD_SUPER, name );
            }
        }

        for ( MetaData child : md.getChildren() ) {
            MetaDataModel cm = (MetaDataModel) m.getMetaData().newInstance();

            populateModel( cm, child, false );

            List<MetaDataModel> mkids = m.getObjectArray( MetaDataModel.class, MetaDataModel.FIELD_CHILDREN);
            if (mkids == null)  {
                mkids = new ArrayList<>();
                m.setObjectArray( MetaDataModel.FIELD_CHILDREN, mkids );
            }
            mkids.add(cm);
        }
    }
